...
- Add multi-account, pure-HTTP QR login for WeChat Channels with encrypted server-side sessions.
...
- Add public HTTPS access, bounded retention, encrypted credential and customer-content storage, same-account worker uniqueness, idempotency, and a service-wide emergency stop as correctness protections; do not import AIDCP business strategies.

## Capabilities

### New Capabilities

- `multi-user-qr-auth`: Browserless QR authorization, encrypted session persistence, global demo-session selection, expiry, logout, and same-account worker uniqueness.
...
- Public access is provided only through an HTTPS boundary. Compatible origins use SSE; the public Quick Tunnel path uses a bounded five-second authoritative snapshot-polling fallback.
