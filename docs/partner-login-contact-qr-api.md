...
- 若扫码的视频号已被另一个账号托管，本次登录仍然成功：新账号成为该视频号唯一的托管容器，旧账号自动退役。
- 旧账号退役后会从 `GET /accounts` 列表消失，按原 `accountId` 访问返回 `404 partner_account_not_found`；调用方应将保存的 `accountId` 更新为新账号。
- 旧账号已配置的回复方式、招聘工号和业务微信二维码会自动带到新账号，无需重新提交。
- 旧账号排队中的自动回复会以 `account_superseded` 终止，不会由旧登录代发。
